...
METHOD(plugin_t, get_features, int,
       private_android_plugin_t *this, plugin_feature_t *features[])
{
        static plugin_feature_t f[] = {
                PLUGIN_NOOP,
                        PLUGIN_PROVIDE(CUSTOM, "android"),
        };
        *features = f;
        return countof(f);
}
...
plugin_t *android_plugin_create()
{
	...
	this->handler = android_handler_create(this->service != NULL, this->service);
