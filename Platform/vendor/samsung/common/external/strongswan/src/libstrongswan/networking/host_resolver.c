	...
	while (TRUE)
	{
		...
		{	/* result is a linked list, but we use only the first address */
			...
			if (query->family == AF_UNSPEC)
			{
				...
				{
					if (r->ai_family == AF_INET) // We use only V4 address
					{
						DBG1(DBG_LIB, "AF_UNSPEC - resolved an IPv4 address");
						query->result = host_create_from_sockaddr(r->ai_addr);
						break;
					}
				}
			}
			if (query->result == NULL)
			{
				...
				if (result->ai_family == AF_INET6) DBG1(DBG_LIB, "resolved an IPv6 address");
				else DBG1(DBG_LIB, "resolved an IPv4 address");
			}
#else
			query->result = host_create_from_sockaddr(result->ai_addr);
#endif
